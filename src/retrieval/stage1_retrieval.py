...
from preprocessing.text_normalizer import SanskritPreprocessor
...
class Stage1Retriever:
    ...
    def build_index(self, csv_path: str, text_column: str = 'text', save_path: Optional[str] = None):
        ...
        for idx, row in df.iterrows():
            ...
            if len(text.strip()) > 10:  # Skip very short texts
                ...
                # Use lemmatized text for indexing
                processed_text = processed['lemmas_text']
                if not processed_text.strip():
                    processed_text = processed['normalized']['ascii']
    ...
    def retrieve(self, query: str, top_k: int = 100, preprocess_query: bool = True) -> List[Dict]:
        ...
        if preprocess_query:
            ...
            # Fallback to ASCII if lemmatization fails
            if not search_query.strip():
                search_query = processed_query['normalized']['ascii']
        else:
            search_query = query
